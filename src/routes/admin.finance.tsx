...
function FinanceAdmin() {
    ...
    (async () => {
      ...
      const [c, m, p, s] = await Promise.all([
        supabase
          ...
          .select("id,title,contract_type,status,client_id,property_id,created_at")
          .order("created_at", { ascending: false })
          .limit(100),
        supabase
          .from("mortgage_applications")
          .select("id,full_name,phone,email,monthly_income,status,created_at")
          .order("created_at", { ascending: false })
          .limit(100),
  ...
  const stats = useMemo(() => {
    ...
    return {
      contractsValue,
      commissionTotal,
      contractsActive: contracts.filter((c) => c.status === "signed" || c.status === "active")
        .length,
        ...
        <>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            <Stat
              ...
              value={fmt(stats.contractsValue)}
            />
            <Stat
              icon={<Coins className="h-5 w-5" />}
              label={`Комисионни (${(commissionRate * 100).toFixed(0)}%)`}
              value={fmt(stats.commissionTotal)}
              accent
            />
            <Stat
              icon={<FileText className="h-5 w-5" />}
              label="Активни / Чернови"
              value={`${stats.contractsActive} / ${stats.contractsDraft}`}
            />
            <Stat
              icon={<Wallet className="h-5 w-5" />}
              label="Ипотечни заявки"
              value={String(stats.mortgageCount)}
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-amber-500/20 bg-[rgba(255,255,255,0.05)] p-4">
            ...
            <span className="text-xs text-amber-100/60">
              (запазва се при загуба на фокус — само админи)
            </span>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Panel title="Последни договори">
              {contracts.length === 0 ? (
                <Empty>Все още няма договори.</Empty>
              ) : (
                <ul className="divide-y divide-amber-500/15">
                  {contracts.slice(0, 10).map((c) => {
                    const prop = c.property_id
                      ? properties.find((p) => p.id === c.property_id)
                      : null;
                      ...
                      <li key={c.id} className="flex items-center justify-between gap-3 py-2.5">
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-amber-100/55">
                            {c.contract_type} · {new Date(c.created_at).toLocaleDateString("bg-BG")}
                          </div>
                        </div>
                        <div className="flex flex-col items-end">
                          <span
                            className={`rounded px-1.5 py-0.5 text-[10px] uppercase ${c.status === "signed" ? "bg-emerald-500/20 text-emerald-200" : c.status === "draft" ? "bg-amber-500/20 text-amber-200" : "bg-sky-500/20 text-sky-200"}`}
                          >
                            {c.status}
                          </span>
                          {value > 0 && (
                            <span className="mt-1 text-xs text-amber-100/70">{fmt(value)}</span>
                          )}
                        </div>
                      </li>
                    );
                  ...
                  {mortgages.slice(0, 10).map((m) => (
                    <li key={m.id} className="flex items-center justify-between gap-3 py-2.5">
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-amber-100/55">
                          {m.phone} · {new Date(m.created_at).toLocaleDateString("bg-BG")}
                        </div>
                      </div>
                      <div className="flex flex-col items-end">
                        <span
                          className={`rounded px-1.5 py-0.5 text-[10px] uppercase ${m.status === "approved" ? "bg-emerald-500/20 text-emerald-200" : m.status === "rejected" ? "bg-rose-500/20 text-rose-200" : "bg-amber-500/20 text-amber-200"}`}
                        >
                          {m.status}
                        </span>
                        {m.monthly_income && (
                          <span className="mt-1 text-xs text-amber-100/70">
                            {fmt(Number(m.monthly_income))}/мес
                          </span>
                        )}
                      </div>
                    </li>
...
function Stat({
  ...
  return (
    <div
      className={`rounded-2xl border p-4 backdrop-blur ${accent ? "border-amber-400/60 bg-gradient-to-br from-amber-500/20 to-amber-300/10" : "border-amber-500/20 bg-[rgba(20,4,8,0.6)]"}`}
    >
      <div className="flex items-center gap-2 text-amber-300">
        {icon}
        <span className="text-xs uppercase text-amber-100/70">{label}</span>
      </div>
