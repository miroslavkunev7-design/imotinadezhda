    ...
    <div className="mx-auto w-full max-w-[1800px] space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-amber-500/25 bg-[rgba(20,4,8,0.65)] p-3">
        <div className="flex items-center gap-2">
          <Link
            to="/admin/settings"
            className="inline-flex items-center gap-1 rounded-lg border border-amber-500/30 bg-amber-500/5 px-2 py-1.5 text-xs text-amber-100 hover:bg-amber-500/15"
          >
            ...
            <Link
              ...
              className={cn(
                ...
                p.slug === slug
                  ...
                  : "bg-amber-500/5 text-amber-100/70 hover:bg-amber-500/15",
              )}
            >
              {p.label}
            </Link>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="rounded-md border border-amber-500/30 p-1.5 text-amber-100 disabled:opacity-40 hover:bg-amber-500/15"
            title="Undo"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1 rounded-md border border-amber-500/30 bg-amber-500/5 px-3 py-1.5 text-xs text-amber-100 hover:bg-amber-500/15"
          >
          ...
          <button
            ...
            disabled={saving}
            className="inline-flex items-center gap-1 rounded-md border border-amber-500/40 bg-amber-500/15 px-3 py-1.5 text-xs text-amber-100 hover:bg-amber-500/25 disabled:opacity-50"
          >
            {saving ? (
          ...
          <button
            ...
            disabled={saving || store.blocks.length === 0}
            className="inline-flex items-center gap-1 rounded-md bg-gradient-to-r from-amber-500 to-amber-400 px-3 py-1.5 text-xs font-semibold text-amber-950 disabled:opacity-50"
          >
        ...
        <div className="rounded-2xl border border-amber-500/25 bg-[rgba(20,4,8,0.65)] p-3">
            ...
            {categories.map((c) => (
              <button
                ...
                className={cn(
                  ...
                  activeCat === c
                    ? "bg-amber-500/25 text-amber-100"
                    : "bg-amber-500/5 text-amber-100/60 hover:bg-amber-500/15",
                )}
              >
          ...
          <div className="max-h-[70vh] space-y-1.5 overflow-y-auto pr-1">
            {BLOCK_REGISTRY.filter((b) => b.category === activeCat).map((b) => (
              <button
                key={b.type}
                onClick={() => store.addBlock(b.type)}
                className="flex w-full items-center gap-2 rounded-md border border-amber-500/20 bg-[rgba(40,10,18,0.6)] px-2 py-2 text-left text-xs text-amber-100 hover:border-amber-400/60"
              >
                <span>{b.emoji}</span>
            ...
            <div className="flex flex-col">
              {store.blocks.map((b, idx) => (
                <div
                  ...
                  className={cn(
                    "relative group cursor-pointer ring-inset",
                    store.selectedId === b.id && "ring-2 ring-amber-500",
                  )}
                >
                  {renderBlock(b)}
                  <div className="absolute right-2 top-2 hidden gap-1 group-hover:flex">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        if (idx > 0) store.moveBlock(idx, idx - 1);
                      }}
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        if (idx < store.blocks.length - 1) store.moveBlock(idx, idx + 1);
                      }}
                      className="rounded bg-black/70 p-1 text-white"
                    >
            ...
            <div className="space-y-2">
              ...
              {selectedDef.controls.map((c) => (
                <div key={c.key} className="space-y-1">
                  <label className="text-[11px] uppercase tracking-wider text-amber-200/70">
                    {c.label}
                  </label>
                  {c.type === "textarea" ? (
                    <textarea
                      value={selected.props[c.key] ?? ""}
                      onChange={(e) => store.updateProps(selected.id, { [c.key]: e.target.value })}
                      rows={3}
                      className="w-full rounded-md border border-amber-500/30 bg-black/30 px-2 py-1 text-xs text-amber-50"
                    />
                  ) : c.type === "color" ? (
                    <div className="flex items-center gap-2">
                      <input
                        type="color"
                        value={selected.props[c.key] ?? "#000000"}
                        onChange={(e) =>
                          store.updateProps(selected.id, { [c.key]: e.target.value })
                        }
                        className="h-7 w-10 rounded border border-amber-500/30 bg-transparent"
                      />
                      <input
                        type="text"
                        value={selected.props[c.key] ?? ""}
                        ...
                        className="flex-1 rounded-md border border-amber-500/30 bg-black/30 px-2 py-1 text-xs text-amber-50"
                      />
                    </div>
                  ) : c.type === "number" ? (
                    <input
                      ...
                      min={c.min}
                      max={c.max}
                      value={selected.props[c.key] ?? 0}
                      onChange={(e) =>
                        store.updateProps(selected.id, { [c.key]: Number(e.target.value) })
                      }
                      className="w-full rounded-md border border-amber-500/30 bg-black/30 px-2 py-1 text-xs text-amber-50"
                    />
                  ) : c.type === "select" ? (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-md rounded-2xl border border-amber-500/30 bg-[rgba(20,4,8,0.95)] p-5">
        ...
        <p className="mt-1 text-xs text-amber-100/60">
          Сложи URL на страница, която искаш да копираш или която да послужи за вдъхновение.
        </p>
        <input
          value={url}
          onChange={(e) => setUrl(e.target.value)}
          placeholder="https://example.com"
          className="mt-3 w-full rounded-md border border-amber-500/30 bg-black/30 px-3 py-2 text-sm text-amber-50"
        />
        <div className="mt-3 flex gap-2">
          <button
            onClick={() => setMode("clone")}
            className={cn(
              "flex-1 rounded-md border p-3 text-xs",
              mode === "clone"
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button
            onClick={onClose}
            disabled={busy}
            className="rounded-md border border-amber-500/25 px-3 py-1.5 text-xs text-amber-100/70"
          >
            Откажи
          </button>
          <button
