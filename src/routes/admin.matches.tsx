...
function MatchesAdmin() {
  ...
  const [filter, setFilter] = useState<"all" | "new" | "contacted" | "interested" | "rejected">(
    "new",
  );
  ...
  const runForAll = async () => {
    ...
    for (const p of props) {
      try {
        const r = await triggerMatchForProperty({ data: { property_id: p.id } });
        count += r.matches;
      } catch {}
    }
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-4xl text-amber-100">
            Съвпадения <Heart className="inline h-7 w-7 text-rose-400" />
          </h1>
          <p className="mt-1 text-sm text-amber-100/60">Автоматично съпоставяне клиент ↔ имот</p>
        </div>
        <Button onClick={runForAll} className="gold-cta-button">
          <Sparkles className="h-4 w-4" /> Пресметни всички
        </Button>
      </header>
      ...
      <div className="flex flex-wrap gap-2">
        {[
          ...
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-sm transition ${filter === t.key ? "bg-amber-500 text-amber-950" : "border border-amber-500/30 text-amber-100/70 hover:bg-amber-500/10"}`}
          >
      ...
      <div className="grid gap-4">
        {filtered.map((m) => (
          <div
            key={m.id}
            className="rounded-2xl border border-amber-500/20 bg-[rgba(255,255,255,0.85)] p-5 text-amber-100"
          >
            <div className="grid gap-5 md:grid-cols-[200px_1fr_auto] md:items-center">
              <div className="flex items-center gap-3">
                {m.properties?.cover_image_url ? (
                  <img
                    src={m.properties.cover_image_url}
                    alt=""
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-xs text-amber-100/60">
                    {m.properties?.cities?.name} • {m.properties?.price} {m.properties?.currency}
                  </span>
                </div>
                <div className="mt-1 text-sm text-amber-100/80">
                  ...
                  {m.clients?.phone && (
                    <a
                      href={`tel:${m.clients.phone}`}
                      className="ml-3 inline-flex items-center gap-1 text-amber-300"
                    >
              ...
              <div className="flex flex-wrap gap-2">
                {m.status === "new" && (
                  <button
                    onClick={() => update(m.id, "contacted")}
                    className="rounded-lg border border-amber-500/30 px-3 py-1.5 text-xs hover:bg-amber-500/10"
                  >
                    Маркирай уведомен
                  </button>
                )}
                {m.status !== "interested" && (
                  <button
                    onClick={() => update(m.id, "interested")}
        ...
        {!filtered.length && (
          <div className="rounded-2xl border border-dashed border-amber-500/30 p-10 text-center text-amber-100/50">
            Няма съвпадения в тази категория. Натисни „Пресметни всички" за да генерираш.
          </div>
        )}
      </div>
    </div>
