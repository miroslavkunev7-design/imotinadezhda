...
function MarketingAdmin() {
  ...
  const loadAudience = async (a: Audience) => {
    let q;
    if (a === "clients")
      q = supabase.from("clients").select("full_name,email").not("email", "is", null);
    else if (a === "inquiries")
      q = supabase.from("inquiries").select("name,email").not("email", "is", null);
    else
      q = supabase.from("mortgage_applications").select("full_name,email").not("email", "is", null);
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-amber-100/60">
          Бюлетини, кампании и история на изпратени имейли.
        </p>
      </header>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="rounded-2xl border border-amber-500/20 bg-[rgba(20,4,8,0.6)] p-4 backdrop-blur">
          <h2 className="mb-3 flex items-center gap-2 font-display text-lg text-amber-100">
            <Send className="h-4 w-4" /> Композирай бюлетин
          </h2>
          ...
          <div className="flex flex-wrap gap-2">
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const res = await sendMarketingEmail({
                    data: { recipients, subject, html, template_name: "newsletter" },
                  });
            ...
            >
              ...
              {sending ? "Изпращане..." : `Изпрати реално (${recipients.length})`}
            </Button>
            <Button asChild variant="outline" disabled={recipients.length === 0}>
              <a href={mailtoUrl}>
                <Mail className="h-4 w-4" /> Mailto
              </a>
            </Button>
          ...
          <p className="mt-2 text-[11px] text-amber-100/50">
            Реалното изпращане използва Resend (RESEND_API_KEY + MARKETING_FROM_EMAIL). Без тях
            получателите се записват в лога като „queued".
          </p>
        </section>
        ...
        <section className="rounded-2xl border border-amber-500/20 bg-[rgba(20,4,8,0.6)] p-4 backdrop-blur">
          ...
          {loading ? (
            ...
            <div className="py-6 text-center text-sm text-amber-100/55">
