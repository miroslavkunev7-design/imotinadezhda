...
function InquiriesAdmin() {
  ...
  const setStatus = async (id: string, status: string) => {
    await supabase
      .from("inquiries")
      .update({ status: status as "new" | "in_progress" | "closed" })
      .eq("id", id);
    load();
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-1 rounded-xl border border-amber-500/20 bg-[rgba(255,255,255,0.6)] p-1">
        <button
          ...
          className={`flex flex-1 items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition ${
            tab === "inquiries"
              ? "bg-gradient-to-r from-primary to-[#7a0d22] text-amber-100"
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            <select
              ...
              className="rounded border border-amber-500/30 bg-[rgba(20,4,8,0.6)] px-3 py-1.5 text-sm text-amber-100"
            >
          ...
          {rows
            ...
            .map((r) => (
              <article
                key={r.id}
                className="rounded-2xl border border-amber-500/15 bg-[rgba(255,255,255,0.85)] p-5 text-amber-100"
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="font-semibold">
                      {r.name} <span className="ml-2 text-sm text-amber-100/60">{r.email}</span>
                    </div>
                    {r.phone && <div className="text-sm text-amber-100/60">{r.phone}</div>}
                    {r.properties?.title && (
                      <div className="mt-1 text-xs text-amber-300">Имот: {r.properties.title}</div>
                    )}
                ...
                <NotesEditor
                  initial={r.notes ?? ""}
                  onSave={(v) => saveNotes("inquiries", r.id, v)}
                />
              </article>
            ))}
      ...
      {tab === "mortgages" && (
        <div className="space-y-3">
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  {m.employer && (
                    <div className="mt-1 text-xs text-amber-100/60">Работодател: {m.employer}</div>
                  )}
                  {m.monthly_income != null && (
                    <div className="text-xs text-amber-100/60">
                      Доход: {m.monthly_income} лв./мес.
                    </div>
                  )}
                  {m.properties?.title && (
                    <div className="mt-1 text-xs text-amber-300">Имот: {m.properties.title}</div>
                  )}
                  <div className="mt-1 text-xs text-amber-100/40">
              ...
              <NotesEditor
                initial={m.notes ?? ""}
                onSave={(v) => saveNotes("mortgage_applications", m.id, v)}
              />
                    ...
                    {m.files.map((f) => (
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="ml-5 truncate text-[10px] text-amber-100/50">
                            {f.file_name}
                          </div>
                        </div>
          ...
          {!mortgages.length && (
            <p className="text-center text-amber-100/40">Няма ипотечни кандидатури</p>
          )}
        </div>
      )}
    </div>
