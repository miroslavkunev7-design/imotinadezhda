...
async function prepareChat(body: z.infer<typeof InputSchema>) {
  // Cast: RPCs `visitor_prepare_customer_chat` / `visitor_save_customer_reply`
  // exist in the DB but are not yet in generated `Database["public"]["Functions"]`.
  const { data, error } = await (safeAdmin.rpc as unknown as (
    fn: string,
    args: Record<string, unknown>,
  ) => Promise<{ data: unknown; error: { message: string } | null }>)("visitor_prepare_customer_chat", {
  ...
  return data as unknown as { chat_id: string; history: Array<{ role: string; content: string }> };
}
...
async function saveReply(visitorToken: string, chatId: string, reply: string) {
  const { error } = await (safeAdmin.rpc as unknown as (
    fn: string,
    args: Record<string, unknown>,
  ) => Promise<{ data: unknown; error: { message: string } | null }>)("visitor_save_customer_reply", {
