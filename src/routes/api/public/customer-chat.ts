    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        try {
          ...
          // Автоматизация №16 — огледално записване в омниканалната инбокс кутия (не блокира отговора).
          try {
            const { ensureConversation } = await import("@/lib/omnibot.server");
            ...
            console.warn("[customer-chat] omnibot mirror skipped:", (mirrorErr as Error)?.message);
          }
