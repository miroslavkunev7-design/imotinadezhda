...
function DatabasePage() {
  ...
  const [quarters, setQuarters] = useState<
    Array<{ id: string; name: string; slug: string; city_id: string; image_url: string | null }>
  >([]);
  ...
  useEffect(() => {
    ...
    supabase
      .from("quarters")
      .select("id, name, slug, city_id, image_url")
      .order("name")
      .then(({ data }) => setQuarters((data as any) ?? []));
  }, []);
          ...
          <div className="rounded-2xl border border-amber-500/20 bg-[rgba(255,251,243,0.97)] p-5 shadow-lg">
            {loading ? (
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                ...
                >
                  <Plus
                    className="h-10 w-10 text-primary/70 group-hover:text-primary"
                    strokeWidth={2.5}
                  />
