...
function QuartersAdmin() {
  ...
  const save = async (e: FormEvent) => {
    ...
    const op = id
      ? supabase.from("quarters").update(p).eq("id", id)
      : supabase.from("quarters").insert(p);
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        ...
        >
          <Plus className="h-4 w-4" /> Нов квартал
        </Button>
      </header>
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-display text-2xl text-accent-foreground">
                {editing.id ? "Редакция" : "Нов квартал"}
              </h2>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <label className="text-sm">
                <div className="mb-1 text-muted-foreground">Град</div>
                <select
                  required
                  value={editing.city_id}
                  onChange={(e) => setEditing({ ...editing, city_id: e.target.value })}
              ...
              <label className="text-sm md:col-span-2">
                <div className="mb-1 text-muted-foreground">Име</div>
                <input
                  required
                  value={editing.name}
              ...
              <label className="text-sm">
                <div className="mb-1 text-muted-foreground">Снимка (URL)</div>
                <input
              ...
              <label className="text-sm">
                <div className="mb-1 text-muted-foreground">Средна цена €/м²</div>
                <input
                  type="number"
                  step="0.01"
                  value={editing.avg_price_per_sqm ?? ""}
                  onChange={(e) =>
                    setEditing({
                      ...editing,
                      avg_price_per_sqm: e.target.value ? Number(e.target.value) : null,
                    })
                  }
                  className="w-full rounded border border-input bg-background px-3 py-2"
                />
              ...
              <label className="text-sm">
                <div className="mb-1 text-muted-foreground">Подредба</div>
                <input
                  type="number"
              ...
              <label className="text-sm md:col-span-2">
                <div className="mb-1 text-muted-foreground">Описание</div>
                <textarea
                  rows={3}
