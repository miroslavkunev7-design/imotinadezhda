        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {sorted.map((r) => {
            ...
            return (
              <div
                key={r.id}
                className="client-card flex flex-col rounded-2xl border border-[#C9A84C]/45 bg-[#faf6ee] p-4 text-left text-[#8B1A2B] shadow-sm transition hover:border-[#C9A84C] hover:shadow-md"
              >
                <button type="button" className="min-w-0 flex-1 text-left text-[#8B1A2B]" onClick={() => guard(r, () => setDetailsFor(r))}>
                  <div className="flex items-start justify-between gap-2">
                    <span className="client-card-badge rounded-full bg-[#8B1A2B]/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-[#8B1A2B]">
                  ...
                  {r.phone ? (
                    <div className="client-card-muted mt-1 flex items-center gap-1.5 text-sm text-[#8B1A2B]">
                      <Phone className="h-3.5 w-3.5" />{r.phone}
                    </div>
                  ) : (
                    <div className="client-card-muted mt-1 text-sm text-[#8B1A2B]">Няма телефон</div>
                  )}
                  <div className="mt-2 flex flex-wrap items-center gap-1.5">
                    ...
                    {r.cities?.name && (
                      <span className="client-card-muted inline-flex items-center gap-1 text-[11px] text-[#8B1A2B]">
                        <MapPin className="h-3 w-3" />{r.cities.name}{r.quarters?.name ? `, ${r.quarters.name}` : ""}
                      </span>
                    )}
                  </div>
                  <div className="client-card-muted mt-2 text-xs text-[#8B1A2B]">
