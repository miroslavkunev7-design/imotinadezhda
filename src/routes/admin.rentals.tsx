...
function RentalsAdmin() {
  ...
  const filtered = rows.filter((r) => {
    ...
    return (
      (r.tenant_name ?? r.tenant?.full_name ?? "").toLowerCase().includes(s) ||
  ...
  return (
    <div className="space-y-6" data-crm-themed>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2 text-sm">
              <div className="font-display text-lg text-amber-100">
                Липсва таблица{" "}
                <code className="rounded bg-black/30 px-1.5 py-0.5">public.{missingTable}</code>
              </div>
              <p className="text-amber-100/90">
                Базата данни още няма нужните таблици за модул „Наеми“. За да заработи, пусни SQL
                миграциите в Supabase Dashboard → SQL Editor:
              </p>
              <ol className="ml-5 list-decimal space-y-1 text-amber-100/85">
                <li>
                  <code>db/migrations/20260713120000_rentals_and_payments.sql</code>
                </li>
                <li>
                  <code>db/migrations/20260713130000_rentals_management_fee.sql</code>
                </li>
              </ol>
              <a
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <input
            ...
            placeholder="Търси наемател / собственик / адрес..."
          ...
          />
          <Button onClick={newRental} className="gold-cta-button">
            <Plus className="h-4 w-4" /> Нов наем
          </Button>
        </div>
      </header>
      ...
      <div className="overflow-x-auto rounded-xl border border-amber-500/15 bg-[rgba(255,255,255,0.9)]">
        <table className="w-full min-w-[900px] text-sm">
          ...
          <tbody className="text-[#3a0f18]">
              ...
              return (
                <tr key={r.id} className="border-t border-amber-500/10 hover:bg-amber-500/5">
                  <td className="px-4 py-2 font-semibold">
                    <div>{tName}</div>
                    {tPhone && (
                      <div className="flex items-center gap-1 text-xs text-[#6b1626]">
                  ...
                  <td className="px-4 py-2 text-xs">
                    {r.cities?.name && (
                      <div>
                        {r.cities.name}
                        {r.quarters?.name ? `, ${r.quarters.name}` : ""}
                      </div>
                    )}
                  ...
                  <td className="px-4 py-2 text-xs">
                    ...
                    {r.payment_day && (
                      <div className="text-[#6b1626]">ден на плащане: {r.payment_day}</div>
                    )}
                  </td>
                  <td className="px-4 py-2 text-xs">
                    {r.monthly_rent ? (
                      <>
                        <div className="font-semibold">
                          Наем: {Number(r.monthly_rent).toFixed(2)} {r.currency}
                        </div>
                        <div className="text-[#6b1626]">
                          Наша комисиона: {Number(r.management_fee ?? 0).toFixed(2)} {r.currency}
                        </div>
                        <div className="font-semibold text-emerald-800">
                          За собственика:{" "}
                          {(Number(r.monthly_rent) - Number(r.management_fee ?? 0)).toFixed(2)}{" "}
                          {r.currency}
                        </div>
                      </>
                    ) : (
                    ...
                    {r.deposit ? (
                      <div className="mt-1 text-[#6b1626]">депозит: {r.deposit}</div>
                    ) : null}
                  </td>
                  <td className="px-4 py-2 text-xs">
                    <span
                      className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${r.status === "active" ? "bg-emerald-100 text-emerald-800" : "bg-neutral-200 text-neutral-700"}`}
                    >
                      {r.status}
                    </span>
                  </td>
                  <td className="px-4 py-2 text-right whitespace-nowrap">
                    <Button
                      ...
                      className="gold-cta-button mr-2"
                    >
                      Плащания
                    </Button>
                    <button
                      className="mr-2 text-[#8B1A2B]"
                      title="Редакция"
                      onClick={() => setEditing(r)}
                    >
                      <Pencil className="h-4 w-4" />
                    </button>
                    <button className="text-rose-600" onClick={() => remove(r.id)}>
            ...
            {!filtered.length && (
              <tr>
                <td colSpan={7} className="px-4 py-10 text-center text-[#8B1A2B]/60">
                  Няма договори за наем. Добави нов.
                </td>
              </tr>
            )}
          </tbody>
      ...
      {openId && (
        <PaymentsModal rental={rows.find((r) => r.id === openId)} onClose={() => setOpenId(null)} />
      )}
    </div>
  );
}
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-2xl text-primary">
            {rental.id ? "Редакция на наем" : "Нов договор за наем"}
          </h2>
        ...
        <Section title="Наемател">
          <div className="grid gap-3 md:grid-cols-2">
            <Field label="От клиенти (незадължително)">
              <select
                value={rental.tenant_client_id ?? ""}
                onChange={(e) => onChange({ ...rental, tenant_client_id: e.target.value || null })}
                className={iC}
              >
            ...
            <Field label="Име">
              <input
                ...
                onChange={(e) => onChange({ ...rental, tenant_name: e.target.value })}
                className={iC}
              />
            </Field>
            <Field label="Телефон">
              <input
                ...
                onChange={(e) => onChange({ ...rental, tenant_phone: e.target.value })}
                className={iC}
              />
            </Field>
          </div>
        ...
        <Section title="Собственик / Наемодател">
          <div className="grid gap-3 md:grid-cols-2">
            <Field label="От клиенти (незадължително)">
              <select
                value={rental.landlord_client_id ?? ""}
                onChange={(e) =>
                  onChange({ ...rental, landlord_client_id: e.target.value || null })
                }
                className={iC}
              >
                <option value="">— свободен текст —</option>
            ...
            <Field label="Име">
              <input
                value={rental.landlord_name ?? ""}
                onChange={(e) => onChange({ ...rental, landlord_name: e.target.value })}
                className={iC}
              />
            </Field>
            <Field label="Телефон">
              <input
                value={rental.landlord_phone ?? ""}
                onChange={(e) => onChange({ ...rental, landlord_phone: e.target.value })}
                className={iC}
              />
            </Field>
          </div>
        ...
        <Section title="Имот и договор">
          <div className="grid gap-3 md:grid-cols-2">
            <Field label="Адрес / улица">
              <input
                value={rental.address ?? ""}
                onChange={(e) => onChange({ ...rental, address: e.target.value })}
                className={iC}
              />
            </Field>
            <Field label="Статус">
              <select
                ...
                onChange={(e) => onChange({ ...rental, status: e.target.value })}
                className={iC}
              >
                <option value="active">Активен</option>
                <option value="ended">Прекратен</option>
            ...
            <Field label="Начална дата">
              <input
                ...
                onChange={(e) => onChange({ ...rental, start_date: e.target.value || null })}
                className={iC}
              />
            </Field>
            <Field label="Крайна дата">
              <input
                ...
                onChange={(e) => onChange({ ...rental, end_date: e.target.value || null })}
                className={iC}
              />
            </Field>
            <Field label="Месечен наем">
              <input
                type="number"
                step="0.01"
            ...
            <Field label="Валута">
              <select
                value={rental.currency ?? "EUR"}
                onChange={(e) => onChange({ ...rental, currency: e.target.value })}
                className={iC}
              >
            ...
            <Field label="Наша комисиона / месец">
              <input
                type="number"
                step="0.01"
                value={rental.management_fee ?? ""}
                onChange={(e) =>
                  onChange({
                    ...rental,
                    management_fee: e.target.value ? Number(e.target.value) : 0,
                  })
                }
                className={iC}
              />
            </Field>
            <Field label="За собственика (авт.)">
              <input
                readOnly
                value={
                  rental.monthly_rent
                    ? (Number(rental.monthly_rent) - Number(rental.management_fee ?? 0)).toFixed(2)
                    : ""
                }
                className={iC + " bg-emerald-50 font-semibold"}
              />
            </Field>
            <Field label="Ден на плащане (1–31)">
              <input
                type="number"
                min={1}
                max={31}
                value={rental.payment_day ?? ""}
                onChange={(e) =>
                  onChange({
                    ...rental,
                    payment_day: e.target.value ? Number(e.target.value) : null,
                  })
                }
                className={iC}
              />
            ...
            <Field label="Депозит">
              <input
                ...
                onChange={(e) =>
                  onChange({ ...rental, deposit: e.target.value ? Number(e.target.value) : null })
                }
                className={iC}
              />
            </Field>
        ...
        <Field label="Опис на движими вещи">
          <textarea
            rows={3}
            value={rental.inventory ?? ""}
            onChange={(e) => onChange({ ...rental, inventory: e.target.value })}
            className={iC}
          />
        </Field>
        <Field label="Бележки">
          <textarea
            rows={2}
            value={rental.notes ?? ""}
            onChange={(e) => onChange({ ...rental, notes: e.target.value })}
            className={iC}
          />
        </Field>
...
function PaymentsModal({ rental, onClose }: { rental: any; onClose: () => void }) {
  ...
  const load = async () => {
    try {
      setRows((await listRentalPayments({ data: { rental_id: rental.id } })) as any[]);
    } catch (e: any) {
      toast.error(e.message);
    }
  };
  ...
  const newPayment = () => {
    ...
    setEditing({
      ...
      due_date: rental.payment_day
        ? `${yyyy}-${mm}-${String(rental.payment_day).padStart(2, "0")}`
        : null,
      paid_date: null,
  ...
  const remove = async (id: string) => {
    ...
    try {
      await deleteRentalPayment({ data: { id } });
      await load();
    } catch (e: any) {
      toast.error(e.message);
    }
  ...
  return (
    <Modal onClose={onClose} wide>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <div className="text-xs uppercase tracking-widest text-[#8B1A2B]/70">
            Месечни плащания
          </div>
          <h2 className="font-display text-2xl text-primary">
            {rental?.tenant?.full_name ?? rental?.tenant_name ?? "Наемател"} ·{" "}
            {rental?.address ?? ""}
          </h2>
          <div className="mt-1 text-xs text-[#3a0f18]">
            Наем: {rental?.monthly_rent ?? "—"} {rental?.currency} · Наша комисиона:{" "}
            {rental?.management_fee ?? 0} {rental?.currency} · За собственика:{" "}
            {rental?.monthly_rent
              ? (Number(rental.monthly_rent) - Number(rental.management_fee ?? 0)).toFixed(2)
              : "—"}{" "}
            {rental?.currency} · Ден на плащане: {rental?.payment_day ?? "—"}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button onClick={newPayment} className="gold-cta-button">
            <Plus className="h-4 w-4" /> Ново плащане
          </Button>
          <button type="button" onClick={onClose}>
            <X className="h-5 w-5" />
          </button>
      ...
      <div className="mb-3 grid gap-3 md:grid-cols-4">
        <StatBox
          label="Платени"
          value={`${totals.paid.toFixed(2)} ${rental?.currency ?? ""}`}
          color="emerald"
        />
        <StatBox
          label="Неплатени / очаквани"
          value={`${totals.unpaid.toFixed(2)} ${rental?.currency ?? ""}`}
          color="rose"
        />
        <StatBox
          label="Наши комисиони (платени)"
          value={`${totals.commissionPaid.toFixed(2)} ${rental?.currency ?? ""}`}
          color="neutral"
        />
        <StatBox
          label="За собственика (нето)"
          value={`${totals.netPaid.toFixed(2)} ${rental?.currency ?? ""}`}
          color="emerald"
        />
      </div>
          ...
          <tbody>
            {rows.map((p) => (
              <tr key={p.id} className="border-t border-amber-500/10 hover:bg-amber-500/5">
                ...
                <td className="px-4 py-2 text-xs">
                  {p.document_url ? (
                    <a
                      ...
                      className="inline-flex items-center gap-1 text-[#8B1A2B] hover:underline"
                    >
                      <Download className="h-3 w-3" />
                      {p.document_name ?? "файл"}
                    </a>
                  ) : (
                    "—"
                  )}
                ...
                <td className="px-4 py-2 text-right whitespace-nowrap">
                  <button className="mr-2 text-[#8B1A2B]" onClick={() => setEditing(p)}>
                    <Pencil className="h-4 w-4" />
                  </button>
            ...
            {!rows.length && (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-[#8B1A2B]/60">
                  Няма записани плащания. Добави ново.
                </td>
              </tr>
            )}
          </tbody>
        ...
        <div className="mt-4 rounded-xl border-2 border-[#C9A84C]/60 bg-[rgba(255,251,243,0.98)] p-4">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="font-display text-lg text-primary">
              {editing.id ? "Редакция на плащане" : "Ново плащане"}
            </h3>
            <button onClick={() => setEditing(null)}>
              <X className="h-5 w-5 text-[#8B1A2B]" />
            </button>
          </div>
          <form onSubmit={save} className="space-y-3">
            <div className="grid gap-3 md:grid-cols-3">
              <Field label="Месец (YYYY-MM)">
                <input
                  required
                  value={editing.period_month?.slice(0, 7) ?? ""}
                  onChange={(e) => setEditing({ ...editing, period_month: e.target.value })}
                  placeholder="2026-07"
                  className={iC}
                />
              </Field>
            ...
            <div className="flex flex-wrap items-center gap-3 rounded-lg border-2 border-dashed border-[#C9A84C]/50 bg-[rgba(201,168,76,0.08)] p-3">
              <label className="flex cursor-pointer items-center gap-2 rounded-lg bg-[#8B1A2B] px-4 py-2 text-white hover:bg-[#6b1626]">
                ...
                <span className="text-sm">{busy ? "Качване…" : "Прикачи снимка / PDF"}</span>
                <input
                  type="file"
              ...
              {editing.document_url ? (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-sm text-[#8B1A2B] hover:underline"
                >
                  <FileText className="h-4 w-4" />
                  {editing.document_name ?? "прикачен файл"}
                </a>
              ) : (
...
function Modal({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-[#8B1A2B]/60 p-4"
      onClick={onClose}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className={`max-h-[94vh] w-full ${wide ? "max-w-5xl" : "max-w-3xl"} overflow-auto rounded-2xl bg-[rgba(255,251,243,0.98)] p-6 shadow-2xl`}
      >
        {children}
      </div>
    </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1 block text-xs font-medium uppercase tracking-wide text-[#8B1A2B]/80">
        {label}
      </span>
      {children}
    </label>
