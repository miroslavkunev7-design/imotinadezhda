  ...
  head: () => ({
    meta: [
      { title: "За нас — Имоти Надежда" },
      {
        name: "description",
        content:
          "Имоти Надежда — луксозна агенция за недвижими имоти в Шумен, Варна, Бургас и Нови пазар.",
      },
      { property: "og:title", content: "За нас — Имоти Надежда" },
      {
        property: "og:description",
        content: "Водеща агенция за недвижими имоти с 15+ години опит.",
      },
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#fbf6ea] to-white">
      ...
      <div className="mx-auto max-w-6xl px-4 pb-16 pt-8 md:px-8">
        <section className="grid gap-8 md:grid-cols-2 md:items-center">
          <div>
            <p className="font-display text-xs uppercase tracking-[0.32em] text-[#C9A84C]">
              Имоти Надежда
            </p>
        ...
        <div className="mt-12 grid grid-cols-2 gap-3 rounded-2xl border border-[#C9A84C]/40 bg-white p-6 md:grid-cols-4">
          {STATS.map((s) => (
            <div key={s.label} className="text-center">
              <div className="font-display text-3xl font-bold text-[#8B1A2B] md:text-4xl">
                {s.value}
              </div>
              <div className="mt-1 text-[11px] uppercase tracking-widest text-[#2b1418]/60">
                {s.label}
              </div>
            </div>
          ))}
        ...
        <section className="mt-16">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {TEAM.map((p) => (
              <div
                key={p.name}
                className="rounded-2xl border border-[#C9A84C]/40 bg-white p-5 shadow-sm"
              >
                ...
                <div className="mt-3 space-y-1 text-center text-xs">
                  <a
                    ...
                    className="block text-[#8B1A2B] hover:underline"
                  >
                    {p.phone}
                  </a>
                  <a href={`mailto:${p.email}`} className="block text-[#2b1418]/70 hover:underline">
                    {p.email}
                  </a>
                </div>
              </div>
