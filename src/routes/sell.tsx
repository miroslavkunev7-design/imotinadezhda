  ...
  head: () => ({
    meta: [
      { title: "Продай имот — Имоти Надежда" },
      {
        name: "description",
        content:
          "Продайте имота си чрез Имоти Надежда — безплатна оценка и професионална обработка.",
      },
      { property: "og:title", content: "Продай имот — Имоти Надежда" },
      {
        property: "og:description",
        content: "Безплатна оценка и професионална обработка на обявата.",
      },
...
function SellPage() {
  ...
  const inputCls =
    "w-full rounded-lg border border-[#C9A84C]/40 bg-[#fbf6ea] px-3 py-2 text-sm text-[#2b1418] outline-none focus:border-[#8B1A2B]";

  return (
    <div className="min-h-screen bg-gradient-to-b from-[#fbf6ea] to-white">
      <SiteHeader />
      <div className="mx-auto max-w-3xl px-4 pb-16 pt-8 md:px-8">
        ...
        <p className="mt-2 text-[15px] text-[#2b1418]/80">
          Попълнете формата и нашият екип ще се свърже с Вас за безплатна оценка и публикуване на
          обявата.
        </p>

        <form
          onSubmit={submit}
          className="mt-8 grid gap-3 rounded-2xl border border-[#C9A84C]/40 bg-white p-6 sm:grid-cols-2"
        >
          <input
            required
            placeholder="Име *"
          ...
          <button
            type="submit"
            disabled={sending}
            className="sm:col-span-2 rounded-lg bg-[#8B1A2B] px-4 py-2.5 text-sm font-semibold text-white shadow-lg transition hover:brightness-110 disabled:opacity-60"
          >
