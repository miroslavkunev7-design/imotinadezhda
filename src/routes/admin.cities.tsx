...
function CitiesAdmin() {
  ...
  const runBackfill = async () => {
    ...
    try {
      ...
      for (let i = 0; i < 30; i++) {
        const r = (await backfillFn({ data: { limit: 40 } } as never)) as {
          processed: number;
          geocoded: number;
          remaining: number;
        };
  ...
  const save = async (e: FormEvent) => {
    ...
    const op = id
      ? supabase.from("cities").update(payload).eq("id", id)
  ...
  const remove = async (id: string) => {
    if (!confirm("Изтриване на града? Всички квартали и имоти, свързани с него, ще се повлияят."))
      return;
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            disabled={geocoding}
            variant="outline"
            title="Геокодира селата чрез OpenStreetMap и изчислява разстояние до града"
          >
            <Compass className="h-4 w-4" />{" "}
            {geocoding ? geoProgress || "Геокодиране…" : "Геокодирай селата"}
          </Button>
          <Button
            onClick={() =>
              setEditing({ slug: "", name: "", display_order: rows.length, is_published: true })
            }
            className="gold-cta-button"
          >
            <Plus className="h-4 w-4" /> Нов град
          </Button>
        </div>
      </header>
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/50 text-left">
            <tr>
              ...
              <th className="px-4 py-3">Население</th>
      ...
      {editing && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-[#8B1A2B]/50 p-4"
          onClick={() => setEditing(null)}
        >
          <form
            onClick={(e) => e.stopPropagation()}
            onSubmit={save}
            className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-2xl bg-card p-6 shadow-2xl"
          >
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-display text-2xl text-accent-foreground">
                {editing.id ? "Редакция" : "Нов град"}
              </h2>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <Lbl label="Hero видео (URL .mp4/.webm)" className="md:col-span-2">
                <input
                  value={editing.hero_video_url ?? ""}
                  onChange={(e) =>
                    setEditing({ ...editing, hero_video_url: e.target.value || null })
                  }
              ...
              <Lbl label="Население">
                <input
                  ...
                  onChange={(e) =>
                    setEditing({
                      ...editing,
                      population: e.target.value ? Number(e.target.value) : null,
                    })
                  }
                  className="inp"
                />
              ...
              <Lbl label="Площ (км²)">
                <input
                  type="number"
                  step="0.01"
                  value={editing.area_km2 ?? ""}
                  onChange={(e) =>
                    setEditing({
                      ...editing,
                      area_km2: e.target.value ? Number(e.target.value) : null,
                    })
                  }
                  className="inp"
                />
              ...
              <Lbl label="Описание" className="md:col-span-2">
                <textarea
                  rows={3}
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              <Button type="submit" className="gold-cta-button">
                Запази
              </Button>
            </div>
          </form>
