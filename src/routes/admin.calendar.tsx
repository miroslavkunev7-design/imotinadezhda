...
function CalendarAdmin() {
  ...
  const cells = useMemo(() => {
    ...
    for (let d = 1; d <= days; d++)
      arr.push({ date: new Date(now.getFullYear(), now.getMonth(), d) });
  ...
  const selectedEvents = selected
    ? events.filter((e) => new Date(e.date).toDateString() === new Date(selected).toDateString())
    : [];
    ...
    <div className="space-y-6">
      <header className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => setNow(new Date(now.getFullYear(), now.getMonth() - 1, 1))}
      ...
      <div className="rounded-2xl border border-amber-500/20 bg-[rgba(20,4,8,0.6)] p-4 backdrop-blur">
        ...
        <div className="grid grid-cols-7 gap-1">
          {cells.map((c, i) => {
            if (!c.date)
              return <div key={i} className="aspect-square rounded-md bg-[#3a0912]/40" />;
            ...
            return (
              <button
                ...
                } ${selected && new Date(selected).toDateString() === c.date.toDateString() ? "ring-2 ring-[#C9A84C]" : ""}`}
              >
                ...
                <div className="mt-0.5 space-y-0.5 overflow-hidden">
                  {ev.slice(0, 2).map((e) => (
                    <div
                      key={e.id}
                      className={`truncate rounded px-1 text-[9px] font-bold ${e.kind === "task" ? "bg-emerald-400 text-emerald-950" : "bg-sky-300 text-sky-950"}`}
                    >
                      {e.title}
                    </div>
                  ))}
                  {ev.length > 2 && (
                    <div className="text-[9px] font-bold text-[#C9A84C]">+{ev.length - 2}</div>
                  )}
                </div>
              </button>
            );
      ...
      {selected && (
        <div className="rounded-2xl border border-amber-500/20 bg-[rgba(20,4,8,0.6)] p-4 backdrop-blur">
          <h3 className="mb-3 font-display text-lg text-amber-100">
            {new Date(selected).toLocaleDateString("bg-BG", {
              weekday: "long",
              day: "numeric",
              month: "long",
              year: "numeric",
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-amber-100/60">
                      {new Date(e.date).toLocaleTimeString("bg-BG", {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}
                      {e.meta && ` · ${e.meta}`}
