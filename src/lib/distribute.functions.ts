...
export const startFacebookConnect = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await gate(context);
    const fb = await import("@/lib/facebook-oauth.server");
    const { id, ready } = fb.metaApp();
    if (!ready) {
      throw new Error(
    ...
    const state = fb.signFacebookState({ t: Date.now(), origin, uid: context.userId });
    const url = new URL("https://www.facebook.com/v21.0/dialog/oauth");
    url.searchParams.set("client_id", id);
    url.searchParams.set("redirect_uri", fb.facebookRedirectUri(origin));
    url.searchParams.set("state", state);
    url.searchParams.set("scope", fb.FB_OAUTH_SCOPES);
