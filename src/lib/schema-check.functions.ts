...
const EXPECTED: Array<{
...
}> = [
  {
    ...
    purpose: "Профили на потребители / брокери",
  },
  ...
  {
    table: "clients",
    columns: ["id", "full_name", "phone", "client_type"],
    purpose: "Клиенти (CRM)",
  },
  {
    table: "inquiries",
    columns: ["id", "name", "phone", "message"],
    purpose: "Запитвания от сайта",
  },
  ...
  { table: "audit_log", columns: ["id", "action", "actor_id", "created_at"], purpose: "Одит лог" },
  {
    ...
    migration:
      "db/migrations/20260713120000_rentals_and_payments.sql (+ ..._rentals_management_fee.sql)",
...
function looksLikeMissingRelation(msg: string) {
  return (
    ...
    /relationship .* not found/i.test(msg) ||
...
const EXPECTED_FKS: Record<
  string,
  Array<{ column: string; refTable: string; refColumn: string }>
> = {
  rentals: [
  ...
  rental_payments: [{ column: "rental_id", refTable: "rentals", refColumn: "id" }],
};
