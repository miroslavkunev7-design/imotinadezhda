...
export async function searchPublishedProperties(db: Db, args: Record<string, unknown>) {
  ...
  let q = db
    .from("properties")
    .select(
      "id, title, price, currency, area_sqm, rooms, floor, address, cities(name), quarters(name)",
    )
...
export async function fallbackCustomerReply(db: Db, message: string, propertyInfo?: string) {
  ...
  if (
    /имот|апартамент|къща|наем|продажб|бюджет|€|лв|град|квартал|варна|бургас|шумен|нови пазар/i.test(
      text,
    )
  ) {
    const props = await searchPublishedProperties(db, {
      city: text.match(/варна/i)
        ? "Варна"
        : text.match(/бургас/i)
          ? "Бургас"
          : text.match(/шumen/i)
    ...
    if (props.results?.length) {
      const lines = props.results
        .slice(0, 3)
        .map((r: any) => `• [${r.title}](${r.url}) — ${r.price}`);
