  ...
  .handler(async ({ context }): Promise<DnsCheckResult> => {
    ...
    for (const domain of DOMAINS) {
      const [cf, gg] = await Promise.all([
        query("cloudflare", domain, "A"),
        query("google", domain, "A"),
      ]);
    ...
    const [cfTxt, ggTxt] = await Promise.all([
      query("cloudflare", TXT_NAME, "TXT"),
      query("google", TXT_NAME, "TXT"),
    ]);
    ...
    records.push({
      ...
      google: ggTxtVals,
      ok:
        cfTxtVals.some((v) => v.startsWith("vercel-dns-")) &&
        ggTxtVals.some((v) => v.startsWith("vercel-dns-")),
    });
