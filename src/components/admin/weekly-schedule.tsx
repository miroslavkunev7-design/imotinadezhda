...
const TASK_KINDS: TaskKind[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "budget", label: "Бюджет (€)" },
      {
        ...
        label: "Търси",
        type: "textarea",
        placeholder: "тип, квартал, характеристики",
      },
      { key: "location", label: "Място на срещата" },
    ],
  },
  ...
  {
    key: "contract_notary",
    label: "Нотариус / окончателен договор",
    icon: "⚖️",
    fields: [
      { key: "property_id", label: "Имот (ID)" },
  ...
  {
    key: "advertising",
    label: "Публикуване / актуализация на обява",
    icon: "📢",
    fields: [
      { key: "property_id", label: "Имот (ID)" },
...
const KIND_MAP: Record<string, TaskKind> = TASK_KINDS.reduce(
  (acc, k) => {
    acc[k.key] = k;
    return acc;
  },
  {} as Record<string, TaskKind>,
);
...
export function WeeklySchedule() {
  ...
  return (
    <div className="relative pt-5">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 top-0 z-10 flex justify-around px-6"
      >
        {Array.from({ length: 12 }).map((_, i) => (
          <span
            ...
            style={{
              ...
              boxShadow:
                "inset 0 -2px 2px rgba(0,0,0,0.35), inset 0 2px 2px rgba(255,255,255,0.55), 0 2px 3px rgba(0,0,0,0.25)",
            }}
          />
        ))}
      </div>
      <section className="relative rounded-2xl rounded-t-md border border-[#C9A84C]/40 bg-[#fbf6ea] px-4 pb-3 pt-3 shadow-sm">
        {/* Cover: bright snowy landscape with only the logo — like a company calendar cover */}
        <div
          className="relative -mx-4 -mt-3 mb-0 flex items-center justify-center overflow-hidden rounded-t-md border-b-2 border-[#8B1A2B]"
          style={{
        ...
        >
          <img
            ...
            style={{
              width: "min(42%, 220px)",
              filter:
                "drop-shadow(0 2px 6px rgba(255,255,255,0.85)) drop-shadow(0 2px 4px rgba(0,0,0,0.2))",
            }}
          />
        </div>
        {/* Toolbar row below cover */}
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2 border-b border-[#C9A84C]/40 pb-2 pt-2">
          <h2 className="font-display text-base font-bold text-[#8B1A2B]">Седмичен график</h2>
          <div className="flex items-center gap-1">
            <button
              onClick={() => setWeekStart(addDays(weekStart, -7))}
              className="rounded-md border border-[#8B1A2B] bg-white p-1.5 text-[#8B1A2B] hover:bg-[#8B1A2B] hover:text-white"
            ...
            <button
              ...
              className="rounded-md border border-[#8B1A2B] bg-[#8B1A2B] px-2 py-1 text-xs font-semibold text-white hover:bg-[#6f1322]"
            >
              Днес
            </button>
        ...
        <div className="overflow-x-auto">
          <div
            className="grid w-full min-w-[560px] sm:min-w-[940px]"
          ...
          >
            ...
            {DAY_NAMES.map((name, i) => {
              ...
              return (
                <div
                  key={name}
                  className={`border-b-2 border-[#8B1A2B] px-1 pb-1 pt-1 text-center ${isToday ? "bg-[#C9A84C]/25" : ""}`}
                >
            ...
            {HOURS.map((h) => (
              <div key={h} className="contents">
                <div className="border-t border-[#C9A84C]/30 py-0.5 pr-2 text-right text-[13px] font-black tabular-nums text-[#8B1A2B]">
                ...
                {Array.from({ length: 7 }).map((_, di) => {
                  ...
                  return (
                    <div
                      key={di}
                      className="min-h-[30px] lg:min-h-[64px] border-t border-l border-[#C9A84C]/30 p-0.5 lg:p-1"
                    >
                      {cellTasks.map((t) => {
                        ...
                        return (
                          <div
                            key={t.id}
                            className={`group mb-0.5 rounded-md border px-1 py-0.5 text-[10px] leading-tight lg:mb-1 lg:px-2 lg:py-1 lg:text-[13px] ${t.is_completed ? "border-emerald-400/60 bg-emerald-50 text-emerald-900" : "border-[#8B1A2B]/40 bg-white text-[#2b1418]"}`}
                          >
                            <div className="flex items-start gap-1">
                              <button
                                type="button"
                                onClick={() => toggle(t)}
                                className={`mt-0.5 grid h-3 w-3 flex-none place-items-center rounded border ${t.is_completed ? "border-emerald-500 bg-emerald-500 text-white" : "border-[#8B1A2B]/50 bg-white text-transparent hover:text-[#8B1A2B]/40"}`}
                                aria-label="Готово"
                              >
                              ...
                              <div
                                ...
                                onClick={() => setEditing({ day, hour: h, task: t, mode: "view" })}
                              >
                                <div
                                  className={`whitespace-normal break-words font-semibold ${t.is_completed ? "line-through opacity-70" : ""}`}
                                >
                                  <span
                                    style={
                                      hlBg
                                  ...
                                  >
                                    {getKind(t.auto_action_log?.kind).icon} {t.title}
                                  </span>
                                </div>
                      ...
                      <button
                        ...
                        className="flex w-full items-center justify-center gap-1 rounded-md border border-dashed border-[#C9A84C]/60 py-0 text-[9px] font-semibold text-[#8B1A2B]/70 hover:bg-white hover:text-[#8B1A2B]"
                      >
            ...
            return (
              ...
              >
                <div
                  ...
                  className="my-4 flex max-h-[calc(100dvh-2rem)] w-full max-w-md flex-col rounded-2xl border-2 border-[#C9A84C] bg-[#8B1A2B] shadow-2xl"
                >
                  <div className="flex-1 space-y-3 overflow-y-auto p-5">
                    <div className="flex items-start justify-between border-b border-[#C9A84C]/70 pb-2">
                      <div>
                        ...
                        <div className="mt-1 text-xs font-semibold text-white/85">
                          {editing.day.toLocaleDateString("bg-BG", {
                            weekday: "long",
                            day: "2-digit",
                            month: "short",
                          })}{" "}
                      ...
                      <button
                        type="button"
                        onClick={() => setEditing(null)}
                        className="rounded-md p-1 text-white hover:bg-[#8B1A2B]/10"
                        aria-label="Затвори"
                      >
                        <X className="h-5 w-5" />
                      </button>
                    ...
                    {t.description && (
                      <div className="rounded-md border border-[#8B1A2B]/20 bg-white p-3 text-sm text-black whitespace-pre-wrap">
                        {t.description}
                      </div>
                    )}
                      ...
                      <dl className="grid grid-cols-2 gap-2">
                        {kind.fields.map((f) => {
                          ...
                          return (
                            <div
                              key={f.key}
                              className="rounded-md border border-[#C9A84C]/40 bg-white p-2"
                            >
                              <dt className="text-[10px] font-bold uppercase tracking-wide text-[#8B1A2B]">
                                {f.label}
                              </dt>
                              <dd className="text-sm font-semibold text-black break-words">{v}</dd>
                            </div>
                          );
                        })}
                      </dl>
                    ...
                    {kind.fields.length > 0 && !Object.values(fields).some(Boolean) && (
                      <div className="text-xs italic text-white/85">
                        Няма попълнени полета за тази задача.
                      </div>
                    )}
                  </div>
                  <div className="sticky bottom-0 flex items-center justify-between gap-2 rounded-b-2xl border-t border-[#C9A84C]/70 bg-[#8B1A2B] px-5 py-3">
                    ...
                    <div className="flex gap-2">
                      <button
                        type="button"
                        onClick={() => setEditing({ ...editing, mode: "edit" })}
                        className="rounded-md bg-gradient-to-b from-[#d4b866] to-[#b8942e] px-4 py-2 text-sm font-extrabold text-[#2a0a12] shadow"
                      >
            ...
            return (
              ...
              >
                <form
                  onSubmit={save}
                  onClick={(e) => e.stopPropagation()}
                  className="task-dialog-form my-4 flex max-h-[calc(100dvh-2rem)] w-full max-w-md flex-col rounded-2xl border-2 border-[#C9A84C] bg-[#8B1A2B] shadow-2xl"
                >
                  <div className="flex-1 space-y-3 overflow-y-auto p-5">
                    ...
                    <label className="block">
                      ...
                      <input
                        name="title"
                        required
                        defaultValue={editing.task?.title ?? ""}
                        placeholder={`Например: ${kindDef.label}`}
                        className="mt-1 w-full rounded-md border border-[#8B1A2B]/40 bg-white px-3 py-2 text-sm font-medium text-black outline-none focus:border-[#8B1A2B] focus:ring-2 focus:ring-white/40"
                      />
                    </label>
                    {kindDef.fields.length > 0 && (
                      <div className="rounded-md border border-[#C9A84C]/70 bg-white/60 p-3">
                        <div className="mb-2 text-[11px] font-bold uppercase tracking-wide text-white">
                          Полета за „{kindDef.label}"
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          {kindDef.fields.map((f) => (
                            <label
                              key={f.key}
                              className={`block ${f.type === "textarea" ? "col-span-2" : ""}`}
                            >
                    ...
                    <label className="block">
                      ...
                      <textarea
                        name="description"
                        defaultValue={editing.task?.description ?? ""}
                        rows={2}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <label className="block">
                        ...
                        <select
                          ...
                          defaultValue={
                            editing.task?.due_at
                              ? new Date(editing.task.due_at).getHours()
                              : editing.hour
                          }
                          className="mt-1 w-full rounded-md border border-[#8B1A2B]/40 bg-white px-2 py-2 text-sm font-semibold text-black focus:border-[#8B1A2B] focus:ring-2 focus:ring-white/40"
                        >
                          {HOURS.map((h) => (
                      ...
                      <label className="block">
                        ...
                        <select
                          name="end"
                          defaultValue={
                            editing.task?.auto_action_log?.end_at
                              ? new Date(editing.task.auto_action_log.end_at).getHours()
                              : editing.hour + 1
                          }
                          className="mt-1 w-full rounded-md border border-[#8B1A2B]/40 bg-white px-2 py-2 text-sm font-semibold text-black focus:border-[#8B1A2B] focus:ring-2 focus:ring-white/40"
                        >
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex items-center gap-3">
                        <button
                          type="button"
                          onClick={() => setHlChoice("")}
                          className={`grid h-8 w-8 place-items-center rounded-full border-2 bg-white text-[10px] font-bold text-[#8B1A2B] ${hlChoice === "" ? "border-white ring-2 ring-white/60" : "border-[#8B1A2B]/30"}`}
                        ...
                        {HIGHLIGHTS.map((h) => (
                          <button
                            ...
                            aria-label={h.label}
                            title={h.label}
                            className={`h-8 w-8 rounded-full border-2 transition ${hlChoice === h.key ? "border-white ring-2 ring-white/70 scale-110" : "border-white/80"}`}
                            style={{
                  ...
                  <div className="sticky bottom-0 flex items-center justify-between gap-2 rounded-b-2xl border-t border-[#C9A84C]/70 bg-[#8B1A2B] px-5 py-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        type="submit"
                        disabled={busy}
                        className="rounded-md bg-gradient-to-b from-[#d4b866] to-[#b8942e] px-4 py-2 text-sm font-extrabold text-[#2a0a12] shadow disabled:opacity-50"
                      >
