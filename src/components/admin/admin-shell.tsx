...
import crmHeroVideoMp4 from "@/assets/crm-hero-2026.mp4.asset.json";
import crmHeroVideoWebm from "@/assets/crm-hero-2026.webm.asset.json";
...
const NAV: NavItem[] = [
  ...
  { to: "/admin/photos", label: "Снимки", icon: ImageIcon, badge: "NEW" },
  ...
  { to: "/admin/inquiries", label: "Лийдове", icon: MessageSquare, badge: "NEW" },
  { to: "/admin/clients", label: "Клиенти", icon: Users },
  { to: "/admin/qualify", label: "Квалификация", icon: Gauge, badge: "NEW" },
  { to: "/admin/rentals", label: "Наеми & плащания", icon: Wallet, badge: "NEW" },
  { to: "/admin/brokers", label: "Брокери", icon: UserCog },
  { to: "/admin/bots", label: "Бот-брокери", icon: Bot, badge: "NEW" },
  ...
  { to: "/admin/assistant", label: "Асистент 24/7", icon: Sparkles, badge: "NEW" },
  { to: "/admin/calendar", label: "Календар", icon: Calendar },
  { to: "/admin/viewings", label: "Огледи", icon: KeyRound, badge: "NEW" },
  { to: "/admin/finance", label: "Финанси", icon: Wallet },
  { to: "/admin/marketing", label: "Маркетинг", icon: Megaphone },
  { to: "/admin/distribute", label: "Разпръскване", icon: Share2, badge: "NEW" },
...
export function AdminShell({ children, breadcrumb }: { children: ReactNode; breadcrumb?: string }) {
  ...
  useEffect(() => { setAllowBgVideo(shouldPlayHeroVideo()); }, []);
  ...
  return (
    ...
    >
      {/* Global CRM hero video background — fixed, no crop, no zoom */}
      <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden bg-[#5e0f1d]">
        {allowBgVideo ? (
          <video
            ...
            poster={loginHeroPoster}
          >
            <source src={resolveAssetUrl(crmHeroVideoWebm)} type="video/webm" />
            <source src={resolveAssetUrl(crmHeroVideoMp4)} type="video/mp4" />
          </video>
        ) : (
          <img
            src={loginHeroPoster}
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-[#5e0f1d]/70 via-[#5e0f1d]/55 to-[#3a0912]/80" />
      </div>
        ...
        <div className="flex items-center justify-between gap-3 border-b border-amber-600/25 px-5 py-4">
          <Link to="/" className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-xl ring-1 ring-amber-400/40 shadow-md">
              <img src="/apple-touch-icon.png" alt="Имоти Надежда" className="h-full w-full object-cover" />
            </div>
            <div>
          ...
          >
            <div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-primary text-sm font-bold text-primary-foreground">
              {profileAvatar || user?.user_metadata?.avatar_url ? (
                <img src={(profileAvatar || (user?.user_metadata?.avatar_url as string))} alt="" className="h-full w-full object-cover" loading="lazy" decoding="async" />
              ) : (
      ...
      >
        ...
        {/* Content */}
        <main className="crm-content relative z-10 min-w-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-contain p-3 pb-[calc(6.75rem+env(safe-area-inset-bottom,0px))] md:p-6 lg:p-8 lg:pb-8">
          {children}
        </main>
      </div>
