...
function getDistrictZoneShape(seed: string) {
  let hash = 0;
  for (const char of seed) hash = (hash * 31 + char.charCodeAt(0)) >>> 0;
  const points = Array.from({ length: 8 }, (_, index) => {
    const angle = (Math.PI * 2 * index) / 8;
    const radius = 38 + ((hash >>> (index * 3)) % 18);
  ...
  return {
    ...
    rotate: (hash % 11) - 5,
    scale: 0.92 + ((hash >>> 8) % 8) / 100,
  };
}

function DistrictQuarterMap({
  name,
  cityName,
  citySlug,
  quarterSlug,
  ...
  const query = [name, cityName, "България"].filter(Boolean).join(", ");
  const encodedQuery = encodeURIComponent(query);
  const zone = getDistrictZoneShape(`${citySlug}:${quarterSlug}`);
  const zoneStyle: CSSProperties = {
    clipPath: zone.clipPath,
    transform: `rotateX(58deg) rotateZ(${zone.rotate}deg) scale(${zone.scale})`,
    transformStyle: "preserve-3d",
  };

  return (
    <aside className="relative h-[700px] w-full flex-shrink-0 overflow-hidden rounded-3xl border border-[#c59441] bg-[#25252b] shadow-xl lg:w-80">
      <div className="absolute inset-0 [perspective:900px]">
        <iframe
          title={`Карта на ${name}, ${cityName}`}
          loading="lazy"
          referrerPolicy="no-referrer-when-downgrade"
          className="absolute inset-0 h-full w-full border-0 opacity-45 grayscale"
          src={`https://www.google.com/maps?q=${encodedQuery}&z=14&output=embed`}
        />
        <div
          aria-hidden
          ...
          style={{
            backgroundImage:
              "linear-gradient(90deg, rgba(201,168,76,.22) 1px, transparent 1px), linear-gradient(rgba(201,168,76,.22) 1px, transparent 1px)",
            backgroundSize: "32px 32px",
            mixBlendMode: "screen",
          }}
        />
        <div
          aria-hidden
          className="pointer-events-none absolute inset-0 bg-gradient-to-br from-[#160811]/55 via-transparent to-[#041827]/75"
        />
        <div className="absolute inset-x-6 top-16 bottom-32 flex items-center justify-center">
          <div
            className="relative h-[64%] w-[78%] border-2 border-[#f0d78c] bg-[#a52237]/60 shadow-[0_0_34px_rgba(229,29,72,.65),inset_0_0_28px_rgba(229,29,72,.52)]"
            style={zoneStyle}
          >
            <div
              aria-hidden
              ...
              style={{
                backgroundImage:
                  "linear-gradient(135deg, transparent 48%, rgba(240,215,140,.75) 49%, transparent 50%), linear-gradient(45deg, transparent 48%, rgba(240,215,140,.45) 49%, transparent 50%)",
                backgroundSize: "28px 28px",
              }}
            />
            <div className="absolute inset-0 flex items-center justify-center p-4 text-center">
              <span className="font-serif-nadezhda text-2xl font-bold tracking-[0.12em] text-white drop-shadow-[0_2px_8px_rgba(139,26,43,.8)]">
                {name.toUpperCase()}
              </span>
            </div>
          </div>
        </div>
        <div className="absolute left-5 top-5 rounded-full border border-[#c9a84c]/70 bg-[#2a0810]/80 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-[#f0d78c] backdrop-blur-sm">
          3D зона
        </div>
      </div>
