...
create unique index if not exists crm_page_customizations_user_page_uq
  on public.crm_page_customizations (user_id, page_key) where scope = 'user';
create unique index if not exists crm_page_customizations_global_page_uq
  on public.crm_page_customizations (page_key) where scope = 'global';
...
create policy "own settings select" on public.crm_page_customizations
for select to authenticated
using (scope = 'global' or user_id = auth.uid());

drop policy if exists "own settings insert" on public.crm_page_customizations;
create policy "own settings insert" on public.crm_page_customizations
for insert to authenticated
with check ((scope = 'user' and user_id = auth.uid()) or (scope = 'global' and public.has_role(auth.uid(), 'admin')));

drop policy if exists "own settings update" on public.crm_page_customizations;
create policy "own settings update" on public.crm_page_customizations
for update to authenticated
using ((scope = 'user' and user_id = auth.uid()) or (scope = 'global' and public.has_role(auth.uid(), 'admin')))
with check ((scope = 'user' and user_id = auth.uid()) or (scope = 'global' and public.has_role(auth.uid(), 'admin')));

drop policy if exists "own settings delete" on public.crm_page_customizations;
create policy "own settings delete" on public.crm_page_customizations
